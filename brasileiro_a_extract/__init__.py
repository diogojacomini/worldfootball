...
from shared_code.functions import (check_prlm_temp_round,
                                   ...
                                   read_lake,
                                   get_current_date,)
from shared_code.variables import (CONTAINER_NAME_BRA_A,
                                   CAMADA_RAW_NAME,
                                   CONTAINER_CONTROLE,
                                   PRLM_BRA_A,
...
def main(req: HttpRequest) -> HttpResponse:
    parameters = read_lake(CONTAINER_CONTROLE,
                           f'{get_current_date()}_{PRLM_BRA_A}_prlm.json')
    ...
    filename_output = '{0}/{1}/{1}{2:02}.csv'.format(CAMADA_RAW_NAME, parm1, parm2), CONTAINER_NAME_BRA_A,
    # Load dados brutos
    try:
        upload_lake(df, filename_output, overwrite=True)
